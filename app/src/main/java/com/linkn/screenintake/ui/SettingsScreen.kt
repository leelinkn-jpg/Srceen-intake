...
fun SettingsScreen(
    ...
    Scaffold(
        ...
        Column(
            ...
            StatusRow(Icons.Default.Folder, "保存文件夹", "记录会写进这个文件夹", folderChosen) {
    ...
    Card(
        ...
        Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
            ...
            val tips = listOf(
                ...
                "长按音量上键会在当前界面悬浮弹出一个全屏的拍照取景窗，拍完只用 AI 判断一下这是吃的还是喝的、配一句简单描述，自动存进保存文件夹「日常照片」下的三餐/饮料子目录，「健康」tab 里能看到，不做热量成分这类更深识别，同样不会切走当前 App。",
                ...
                "通知被手滑划掉也不会丢：草稿还在保存文件夹的「待确认」子目录里，几个 Tab 上方也会提示还有几条没处理。",
                "识别不了的（网络错误、截屏失败等）也会把原因存进「待确认」，不会静默丢失。",
                "支出和收入都会自动分类（比如餐饮、交通、工资），分类只在固定的列表里选，「财务」这个 Tab 里能直接看到整体收支和每一笔明细。",
                "待办事项会顺手同步一份到手机系统日历（需要日历权限），可以收到日历自带的提醒；能看出具体时间就用那个时间提醒，看不出来就先建一个「今天」的全天事件，具体几点自己去日历里改一下。"
            )
            tips.forEach { tip ->
                Row {
                    Text(
